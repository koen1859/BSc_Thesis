...
The data set is constructed using the results from \Cref{alg:evaluation}. Each entry of $L$ (the list of TSP path lengths) is a separate observation
...
\begin{enumerate}
	...
	\item Re-run the model with a reduced feature set, only using the most important features.
	      This requires some testing, to analyze what the treshold for dropping out features should be.
\end{enumerate}
