...
It is relevant for last-mile carriers and other logistics companies where efficient
...
For example, when there are clusters of locations with empty space in between
them, or natural boundaries such as canals could have impact on TSP path length in an area. The
BHH formula is not able to capture such complex relations, and might fail to provide accurate results.
...
A potential solution to this might be to use supervised learning methods to predict the length of the TSP path,
...
Using OpenStreetMap \citep{openstreetmap} data, we simulate TSP instances in a wide variety of different urban areas
