...
For example, the prediction error for the Schepenbuurt quarter in Friesland, and the Stad van de Zon quarter in Noord-Holland is especially high.
These areas, among others, have an important feature in common.
Such quarters have a large cluster, with some addresses spread out over the rest of the area. Figure \ref{fig:combined} displays two of such areas.
\begin{figure}[H]
	\centering
...
A possible explanation for the higher prediction errors is that when the number of locations to visit $j$ is small, and (as in these examples) one of the clusters
is much smaller than the other, in the sense of number of addresses in it, that there is a significant probability that no addresses in the smaller cluster are in
the TSP.
...
However, there is also significant probability that there are addresses of the smaller cluster in the TSP, which would in turn lead to a significantly higher path
length.
Based on this, a higher variation in the TSP path length is to be expected, which explains the higher prediction error.
This problem would likely disappear when the number of locations in the TSP $j$ is large enough, since then always locations from the other clusters would be
included as well.
...
Furthermore, it is interesting to consider neighborhoods where the prediction errors are especially low. Figure \ref{fig:combined2} displays maps for two quarters,
...
these clusters are all close to each other, there is not much empty space between them, apart from some small green-space or water. This could explain why the
