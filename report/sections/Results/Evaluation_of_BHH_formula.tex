...
The value for $\beta$ differs strongly from area to area, as expected: some areas have an estimated $\beta$ as low as 1.5, while others have $\beta$ close to 6.
...
A possible explanation for the higher prediction errors is that when the number of locations to visit $j$ is small, and (as in these examples) one of the clusters is much smaller than
